...
def check(persona, name, fn):
    try:
        ...
        waiting = [l for l in lines if l.startswith("- waiting for")][:1]
        msg = " | ".join(lines[:1] + waiting)[:260] or repr(e)
...
with sync_playwright() as p:
    ...
    def a_add_staff():
        ...
        pg.get_by_role("link", name="+ Add staff").click()
        pg.fill("#first", "Kiran")
        pg.fill("#last", "Thapa")
        pg.fill("#username", "kiran")
        pg.fill("#email", "kiran@example.com")
        pg.get_by_role("button", name="United Kingdom").click()
        temp = pg.locator("#temp").input_value()
        pg.get_by_role("button", name="Add staff member").click()
        expect(pg.get_by_text("Kiran can now sign in")).to_be_visible(timeout=5000)
        expect(pg.get_by_text(temp)).to_be_visible()
        c2 = br.new_context(); p2 = c2.new_page()
        p2.goto(W + "/staff/login"); p2.fill("#u", "kiran"); p2.fill("#p", temp)
        p2.get_by_role("button", name="Sign in").click()
        p2.wait_for_url("**/staff/password", timeout=6000)
        p2.fill("#cur", temp); p2.fill("#new", "Kiran-Own-Pass-44"); p2.fill("#conf", "Kiran-Own-Pass-44")
        p2.get_by_role("button", name="Save password").click()
        p2.wait_for_url(W + "/staff", timeout=6000)
        expect(p2.get_by_text("Your students, Kiran Thapa")).to_be_visible(timeout=5000)
        c2.close()
    check("Admin", "Adds a counsellor from the website; they sign in and set a password", a_add_staff)

    def a_reset_and_close():
        pg.goto(W + "/staff/team")
        pg.locator("tr", has_text="Kiran Thapa").get_by_role("link", name="Edit").click()
        expect(pg.get_by_role("heading", name="Kiran")).to_be_visible(timeout=5000)
        pg.get_by_role("button", name="Suggest").click()
        temp = pg.get_by_label("New temporary password").input_value()
        pg.get_by_role("button", name="Reset password").click()
        expect(pg.get_by_text(f"Their temporary password is {temp}")).to_be_visible(timeout=5000)
        c2 = br.new_context(); p2 = c2.new_page()
        p2.goto(W + "/staff/login"); p2.fill("#u", "kiran"); p2.fill("#p", temp)
        p2.get_by_role("button", name="Sign in").click()
        p2.wait_for_url("**/staff/password", timeout=6000)
        c2.close()
        pg.get_by_role("button", name="Close account").click()
        expect(pg.get_by_text("Account closed.")).to_be_visible(timeout=5000)
        pg.goto(W + "/staff/team")
        expect(pg.locator("section", has_text="Closed accounts").get_by_text("Kiran Thapa")).to_be_visible(timeout=5000)
        pg.locator("li", has_text="Kiran Thapa").get_by_role("link", name="View / reopen").click()
        pg.get_by_role("button", name="Reopen account").click()
        expect(pg.get_by_text("Account reopened.")).to_be_visible(timeout=5000)
        # Someone with open students can't just be closed
        pg.goto(W + "/staff/team")
        pg.locator("tr", has_text="Sarita Joshi").get_by_role("link", name="Edit").click()
        expect(pg.get_by_text(re.compile("still have \\d+ open students"))).to_be_visible(timeout=5000)
        assert pg.get_by_role("button", name="Close account").count() == 0
    check("Admin", "Resets a password; closes and reopens an account", a_reset_and_close)

    def a_content():
        pg.goto(W + "/staff/content")
        pg.get_by_role("button", name="+ Add destination").click()
        pg.fill("#f-name", "New Zealand")
        pg.fill("#f-summary", "Post-study work visa and high-quality universities.")
        pg.get_by_role("button", name="Add destination").click()
        expect(pg.get_by_text("/study/new-zealand")).to_be_visible(timeout=5000)
        pg.get_by_role("tab", name="Universities & fees").click()
        pg.select_option("#uni-country", label="New Zealand")
        pg.get_by_role("button", name="+ Add university").click()
        pg.fill("#f-name", "University of Otago")
        pg.fill("#f-city", "Dunedin")
        pg.get_by_role("button", name="Add university").click()
        expect(pg.get_by_text("University of Otago")).to_be_visible(timeout=5000)
        pg.get_by_role("button", name="Courses & fees").click()
        pg.get_by_role("button", name="+ Add course").click()
        pg.fill("#f-name", "Master of Data Science")
        pg.fill("#f-tuition_fee", "45000")
        pg.fill("#f-currency", "NZD")
        pg.get_by_role("button", name="Add course").click()
        expect(pg.get_by_text("NZD 45,000")).to_be_visible(timeout=5000)
        pg.screenshot(path=f"{SHOTS}/admin_content.png", full_page=True)
        pg.goto(W + "/study/new-zealand")
        expect(pg.get_by_text("University of Otago")).to_be_visible(timeout=6000)
        expect(pg.get_by_text(re.compile("NZ\\$45,000"))).to_be_visible()
        # A class with one seat left shows on the home page
        pg.goto(W + "/staff/content")
        pg.get_by_role("tab", name="Language classes").click()
        pg.get_by_role("button", name="+ Add class").click()
        pg.select_option("#f-test_type", "toefl")
        pg.fill("#f-start_date", "2026-10-01")
        pg.fill("#f-seats_taken", "19")
        pg.get_by_role("button", name="Add class").click()
        expect(pg.get_by_text(re.compile("TOEFL · starts"))).to_be_visible(timeout=5000)
        pg.goto(W + "/")
        expect(pg.get_by_text("TOEFL preparation")).to_be_visible(timeout=6000)
        # Hide the destination again
        pg.goto(W + "/staff/content")
        pg.get_by_role("button", name="Edit New Zealand").click()
        pg.get_by_label("Show on the website").uncheck()
        pg.get_by_role("button", name="Save").click()
        expect(pg.get_by_text("Hidden").first).to_be_visible(timeout=5000)
        pg.goto(W + "/study/new-zealand")
        expect(pg.get_by_text("We don't have a page for that destination")).to_be_visible(timeout=6000)
    check("Admin", "Edits website content; public pages update", a_content)

    def a_delete_student():
        pg.goto(W + "/staff")
        pg.get_by_role("button", name="Everyone").click()
        pg.fill("input[placeholder='Search name or phone']", "Extra Student 07")
        pg.wait_for_timeout(900)
        pg.get_by_text("Extra Student 07").click()
        pg.get_by_role("button", name="Delete student…").click()
        btn = pg.get_by_role("button", name="Delete permanently")
        pg.fill("#confirm-name", "extra student 07")
        assert btn.is_disabled()
        pg.fill("#confirm-name", "Extra Student 07")
        btn.click()
        pg.wait_for_url(W + "/staff", timeout=6000)
        pg.get_by_role("button", name="Everyone").click()
        pg.fill("input[placeholder='Search name or phone']", "Extra Student 07")
        pg.wait_for_timeout(900)
        expect(pg.get_by_text("Nothing here yet.")).to_be_visible(timeout=5000)
    check("Admin", "Deletes a student after typing their name", a_delete_student)
    ...
    def a_rename():
        ...
        pg.wait_for_url("**/staff/login")
        login(pg, "admin")
        pg.get_by_role("link", name=re.compile("My account")).click()
        expect(pg.get_by_role("heading", name="My account")).to_be_visible()
        pg.fill("#username", "puja")
        pg.fill("#email", "owner@example.com")
        pg.get_by_role("button", name="Save changes").click()
        pg.wait_for_timeout(600)
        # The box is required, so the browser itself refuses to send an empty one.
        assert pg.locator("#curpw").evaluate("el => el.validity.valueMissing"), "empty password was submitted"
        pg.fill("#curpw", "wrong-one")
        pg.get_by_role("button", name="Save changes").click()
        expect(pg.get_by_text("Enter your current password to change your username.")).to_be_visible(timeout=5000)
        pg.fill("#curpw", "banana123")
        pg.get_by_role("button", name="Save changes").click()
        expect(pg.get_by_text("You sign in as puja.")).to_be_visible(timeout=5000)
        pg.screenshot(path=f"{SHOTS}/admin_my_account.png", full_page=True)
        pg.get_by_role("button", name="Sign out").click()
        pg.wait_for_url("**/staff/login")
        login(pg, "puja")
        expect(pg.get_by_role("link", name="My account (puja)")).to_be_visible(timeout=5000)
        pg.goto(W + "/staff/login")
        pg.fill("#u", "admin"); pg.fill("#p", "banana123")
        pg.get_by_role("button", name="Sign in").click()
        expect(pg.get_by_text("don't match")).to_be_visible(timeout=5000)
    check("Admin", "Changes own username on My account; old name stops working", a_rename)
    c.close()
    br.close()
